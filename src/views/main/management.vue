  ...
  <div class="management" ref="management">
    ...
    >
      ...
      <el-form-item>
        ...
        <el-button
          type="primary"
          size="small"
          @click="handleAdd('editForm')"
          icon="el-icon-plus"
          >新增</el-button
        >
        <el-button type="default" size="small" @click="resetForm('searchForm')"
          >重置</el-button
        >
      </el-form-item>
    </el-form>
    ...
    >
      ...
      <el-table-column fixed="right" label="操作" width="250">
        <template slot-scope="scope">
          <el-button @click="updateMem(scope.row)" type="default" size="mini"
            >编辑</el-button
          >
          <el-button type="danger" size="mini" @click="deleteMem(scope.row)"
            >删除</el-button
          >
        </template>
      </el-table-column>
    ...
    >
      ...
      >
        ...
        <el-form-item
          label="开卡金额"
          prop="money"
          :rules="[{ required: true, message: '开卡金额不能为空' }]"
        >
          <el-input
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="formEdit('editForm')">{{
          isAdd ? "新 增" : "修 改"
        }}</el-button>
        <el-button type="primary" @click="resetForm('editForm')"
          >重 置</el-button
        >
      </div>
    </el-dialog>
...
<script>
import Worker from 'worker-loader!@/unit/test.worker.js';
import {
  getList,
  getMemberList,
  getConditionQuery,
...
export default {
  ...
  methods: {
    ...
    queryMemberList() {
      // 默认查第一页数据
      getMemberList(this.currentPage, this.pageSize).then(
        (res) => {
          const resData = res.data;
          if (resData.code === 200) {
    ...
    submitQuery() {
      ...
      getConditionQuery(this.search).then(
        (res) => {
          const resData = res.data;
          if (resData.code === 200) {
            this.dataList = resData.data.row;
      ...
      this.$refs[name].validate((valid) => {
        if (!valid) {
          ...
          this.isAdd
            ...
            : // 修改会员
              updateMember(this.id, this.form).then(
                (res) => {
                  var resData = res.data;
                  if (resData.code === 200) {
                    this.$message({
